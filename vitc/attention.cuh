...
__global__ void softmax_forward_kernel5(floatX* out, float inv_temperature, const floatX* inp, int N, int T) {
    ...
    for (int i = lane_id; i < T/4 ; i += WARP_SIZE) {
        float regarray[4];
        for (int k = 0; k < 4; ++k) {
    ...
    if(4* (T/4) + lane_id < T) {
        float old_maxval = maxval;
        maxval = fmaxf(maxval, (float)x[4* (T/4) + lane_id]);
        sumval *= expf(inv_temperature * (old_maxval - maxval));
        sumval += expf(inv_temperature * ((float)x[4* (T/4) + lane_id] - maxval));
    }
    ...
    for (int i = lane_id; i < T; i += WARP_SIZE) {
...
__global__ void softmax_autoregressive_backward_inplace_kernel(floatX* datt, const floatX* att,
    ...
    constexpr int T_per_block = 1;

    int t0 = T_per_block*blockIdx.x;
    int idx = blockIdx.y;
...
void attention_backward(floatX* dinp, floatX* dqkvr, floatX* datt, floatX* scratch,
    ...
    softmax_autoregressive_backward_inplace_kernel<<<dim3(T, B * NH), 256>>>(datt, att, B, T, C, scale);
